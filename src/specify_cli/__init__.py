...
def download_template_from_github(ai_assistant: str, download_dir: Path, *, verbose: bool = True, show_progress: bool = True):
    ...
    try:
        response = httpx.get(api_url, timeout=30, follow_redirects=True, verify=False)
    ...
    try:
        with httpx.stream("GET", download_url, timeout=30, follow_redirects=True, verify=False) as response:
            response.raise_for_status()
...
def check():
    ...
    try:
        response = httpx.get("https://api.github.com", timeout=5, follow_redirects=True, verify=False)
